...
out_file2 = f'{out_dir}/Unique_ORFs_region4.fasta'
...
def Interval(df, x, y, z):
    # calculates overlaps of specified orf
    df_int = df[[x, y, z]]
...
def FilteredORF_toFasta():
    ''' Writing filtered ORFs to fasta file for ORF annotation and further analysis '''
    df = CheckOverlappingORFs() # getting df with unique for each candidate orfs
    ids = df['ORF'].to_list()
